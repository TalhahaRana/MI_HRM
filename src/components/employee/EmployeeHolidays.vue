    ...
    const getHolidayData = async () => {
      try {
        const response = await axios.get(
          ...
          {
            params: {
              api_key: "GbwTaQDNPpuPdv8wYvqEpEYfwRJOV5Uo", // Add your correct API key here
              country: "PK", // Specify Pakistan as the country
        ...
        if (holidays.length > 0) {
          ...
          holidayName.value = upcomingHoliday.name;
          holidayDate.value = upcomingHoliday.date.iso;
        } else {
