        ...
        <div class="row">
          ...
          <div class="col-7 row">
            <div class="profile-menu">
              ...
              <div v-if="showDropdown" class="dropdown">
                <EmployeeAnnouncements @close="showDropdown = false" />
              </div>
...
export default {
  ...
  setup() {
    ...
    const router = useRouter(); // Ensure this is defined
    ...
    const unreadCount = computed(() => {
      return store.state.employee.announcements.filter(announcement => !announcement.is_read).length; // Adjust based on your state structure
    });
    ...
    const logout = async () => {
      try {
        localStorage.removeItem("userRole"); // Remove the role from local storage upon logout
        router.push("/"); // Make sure router is defined here
      } catch (error) {
        console.error("Error during logout:", error);
      }
    };
    ...
    return {
      ...
      isHRorEmployee,
      unreadCount,
      router, // Return router if needed elsewhere
    };
  },
};
</script>
